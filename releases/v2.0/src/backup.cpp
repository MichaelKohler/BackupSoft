...
void Backup::initWindow()
{
  ...
  setWindowTitle(tr("Backup running.."));
...
void Backup::initBackup()
{
  ...
  for (int i = 0; i < profiles->length(); i++) {
    ...
    bool isActivated = profileInformation.at(a + 3) == tr("Activated");
    if (isActivated) {
  ...
  size->setText(tr("%1 MB of %2 MB").arg(*current).arg(*total));
  a = 0;
  for (int i = 0; i < profiles->length(); i++) {
    bool isActivated = profileInformation.at(a + 3) == tr("Activated");
    if (isActivated) {
  ...
  if (*total == "0") // HACK: needs another method
    size->setText(tr("Finished copying less than 1 MB.. You can now close the window."));
  else
    size->setText(tr("Finished copying %1 MB.. You can now close the window.").arg(*total));
  ...
  for (int i = 0; i < filesList.length(); i++) {
    ...
    if (isFolder(wholePath)) {
      ...
      if(copySuccess) {
        ...
        size->setText(*current + " MB of " + *total + " MB"); //XXX
      }
    }
  }
}
